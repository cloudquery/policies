{% macro api_server_1_2_5(framework, check_id) %}
  {{ return(adapter.dispatch('api_server_1_2_5')(framework, check_id)) }}
{% endmacro %}

{% macro default__api_server_1_2_5(framework, check_id) %}{% endmacro %}

{% macro postgres__api_server_1_2_5(framework, check_id) %}
...
{% macro snowflake__api_server_1_2_5(framework, check_id) %}
select uid                              AS resource_id,
        '{{framework}}' As framework,
        '{{check_id}}'  As check_id,
        'Ensure that the --kubelet-certificate-authority argument is set as appropriate' AS title,
    ...
    case
      when 
        container.value:command like '%kubelet-certificate-authority%' 
      then 'pass'
      else 'fail'
    end as status
from
...
{% macro bigquery__api_server_1_2_5(framework, check_id) %}
