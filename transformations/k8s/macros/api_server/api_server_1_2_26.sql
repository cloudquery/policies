{% macro api_server_1_2_26(framework, check_id) %}
  {{ return(adapter.dispatch('api_server_1_2_26')(framework, check_id)) }}
{% endmacro %}

{% macro default__api_server_1_2_26(framework, check_id) %}{% endmacro %}

{% macro postgres__api_server_1_2_26(framework, check_id) %}
...
{% macro snowflake__api_server_1_2_26(framework, check_id) %}
select uid                              AS resource_id,
        '{{framework}}' As framework,
        '{{check_id}}'  As check_id,
        'Ensure that the --client-ca-file argument is set as appropriate' AS title,
...
{% macro bigquery__api_server_1_2_26(framework, check_id) %}
