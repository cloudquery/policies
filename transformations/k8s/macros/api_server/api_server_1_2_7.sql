{% macro api_server_1_2_7(framework, check_id) %}
  {{ return(adapter.dispatch('api_server_1_2_7')(framework, check_id)) }}
{% endmacro %}

{% macro default__api_server_1_2_7(framework, check_id) %}{% endmacro %}

{% macro postgres__api_server_1_2_7(framework, check_id) %}
...
{% macro snowflake__api_server_1_2_7(framework, check_id) %}
select uid                              AS resource_id,
        '{{framework}}' As framework,
        '{{check_id}}'  As check_id,
        'Ensure that the --authorization-mode argument includes Node' AS title,
    ...
    case
      ...
      container.value:command like '%authorization-mode=%Node%'
      then 'pass'
      else 'fail'
    end as status
from
...
{% macro bigquery__api_server_1_2_7(framework, check_id) %}
