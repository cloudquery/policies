{% macro job_memory_limit(framework, check_id) %}
  {{ return(adapter.dispatch('job_memory_limit')(framework, check_id)) }}
{% endmacro %}

{% macro default__job_memory_limit(framework, check_id) %}{% endmacro %}

{% macro postgres__job_memory_limit(framework, check_id) %}
