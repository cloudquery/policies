{% macro job_cpu_limit(framework, check_id) %}
  {{ return(adapter.dispatch('job_cpu_limit')(framework, check_id)) }}
{% endmacro %}

{% macro default__job_cpu_limit(framework, check_id) %}{% endmacro %}

{% macro postgres__job_cpu_limit(framework, check_id) %}
