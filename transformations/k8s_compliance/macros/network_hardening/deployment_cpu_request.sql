{% macro deployment_cpu_request(framework, check_id) %}
  {{ return(adapter.dispatch('deployment_cpu_request')(framework, check_id)) }}
{% endmacro %}

{% macro default__deployment_cpu_request(framework, check_id) %}{% endmacro %}

{% macro postgres__deployment_cpu_request(framework, check_id) %}
