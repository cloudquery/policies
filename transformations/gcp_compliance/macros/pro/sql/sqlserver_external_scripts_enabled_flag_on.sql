...
{% macro postgres__sql_sqlserver_external_scripts_enabled_flag_on(framework, check_id) %}
 select
...
{% macro snowflake__sql_sqlserver_external_scripts_enabled_flag_on(framework, check_id) %}
 WITH 
    instance_flags as (
    select
        f.value as value             
