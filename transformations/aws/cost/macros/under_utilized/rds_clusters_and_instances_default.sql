...
WITH rds_cluster_resource_utilization AS (
    ...
    FROM
        ...
        LATERAL jsonb_array_elements(cws.input_json -> 'Dimensions') AS elem
    WHERE
...
rds_instance_resource_utilization AS (
    ...
    FROM
        aws_cloudwatch_metric_statistics cws
        CROSS JOIN LATERAL jsonb_array_elements(cws.input_json -> 'Dimensions') AS elem
    WHERE
        cws.label = 'CPUUtilization'
        AND cws.input_json ->> 'Namespace' = 'AWS/RDS'
...
SELECT
    ...
    c.cost
FROM
    rds_resource_utilization ru
    LEFT JOIN cost_by_region_resource c ON (ru.rds_id = c.line_item_resource_id AND ru.region = c.product_region) OR ru.arn = c.line_item_resource_id
WHERE
    ru.label = 'CPUUtilization'
    AND ru.mean_usage < 50
    AND c.cost > 0
{ % endmacro % }