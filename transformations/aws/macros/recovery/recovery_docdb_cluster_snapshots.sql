{% macro recovery_docdb_cluster_snapshots(framework, check_id) %}
  {{ return(adapter.dispatch('recovery_docdb_cluster_snapshots')(framework, check_id)) }}
{% endmacro %}

{% macro default__recovery_docdb_cluster_snapshots(framework, check_id) %}{% endmacro %}
...
WHERE product_product_name = 'Amazon DocumentDB'
...
{% macro snowflake__recovery_docdb_cluster_snapshots(framework, check_id) %}
