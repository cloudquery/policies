...
{% macro bigquery__elbv2_desync_migration_mode_defensive_or_strictest(framework, check_id) %}
select
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
'Application Load Balancer should be configured with defensive or strictest desync mitigation mode' as title,
  ...
  case
        ...
        ELSE 'fail'
  END as status
from {{ full_table_name("aws_elbv2_load_balancer_attributes") }}
where key = 'routing.http.desync_mitigation_mode'
{% endmacro %}

{% macro athena__elbv2_desync_migration_mode_defensive_or_strictest(framework, check_id) %}
select
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
'Application Load Balancer should be configured with defensive or strictest desync mitigation mode' as title,
