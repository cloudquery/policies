...
{% macro bigquery__waf_global_rulegroup_not_empty(framework, check_id) %}
SELECT
	'{{framework}}' As framework,
    '{{check_id}}' As check_id,
	'A WAF global rule group should have at least one rule' as title,
...
from
  {{ full_table_name("aws_waf_rule_groups") }}
{% endmacro %}

{% macro athena__waf_global_rulegroup_not_empty(framework, check_id) %}
SELECT
	'{{framework}}' As framework,
    '{{check_id}}' As check_id,
	'A WAF global rule group should have at least one rule' as title,
