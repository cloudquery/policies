...
{% macro bigquery__sns_topics_should_be_encrypted_at_rest_using_aws_kms(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'SNS topics should be encrypted at rest using AWS KMS' as title,
...
{% macro athena__sns_topics_should_be_encrypted_at_rest_using_aws_kms(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'SNS topics should be encrypted at rest using AWS KMS' as title,
