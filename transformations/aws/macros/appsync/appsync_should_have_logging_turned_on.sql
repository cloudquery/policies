...
{% macro athena__appsync_should_have_logging_turned_on(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'AWS AppSync should have request-level and field-level logging turned on' as title, 
    ...
    arn as resource_id,
    CASE
        WHEN (json_extract_scalar(log_config, '$.cloudWatchLogsRoleArn') IS NULL OR json_extract_scalar(log_config, '$.cloudWatchLogsRoleArn') = '')
            OR 
            json_extract_scalar(log_config, '$.fieldLogLevel') = 'NONE' THEN 'fail'
        ELSE 'pass'
    END AS status
FROM aws_appsync_graphql_apis
{% endmacro %}