...
{% macro bigquery__autoscale_or_ondemand(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'DynamoDB tables should automatically scale capacity with demand' as title,
    ...
    t.arn AS resource_id,
  CASE
        WHEN JSON_VALUE(t.billing_mode_summary.BillingMode) = 'PAY_PER_REQUEST' then 'pass'
        WHEN (JSON_VALUE(t.billing_mode_summary.BillingMode) = 'PROVISIONED' or JSON_VALUE(t.billing_mode_summary.BillingMode) is NULL )and (pr._cq_id IS not NULL and pw._cq_id IS not NULL) then 'pass'
        ELSE 'fail'
        END
            AS status
FROM
  {{ full_table_name("aws_dynamodb_tables") }} AS t
  LEFT JOIN {{ full_table_name("aws_applicationautoscaling_policies") }} AS pr ON
      pr.service_namespace = 'dynamodb'
      AND pr.resource_id = concat('table/', t.table_name)
	  AND pr.policy_type = 'TargetTrackingScaling'
      AND pr.scalable_dimension = 'dynamodb:table:ReadCapacityUnits'
...
{% macro athena__autoscale_or_ondemand(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'DynamoDB tables should automatically scale capacity with demand' as title,
    ...
    t.arn AS resource_id,
  CASE
        WHEN json_extract_scalar(t.billing_mode_summary, '$.BillingMode') = 'PAY_PER_REQUEST' then 'pass'
        WHEN (json_extract_scalar(t.billing_mode_summary, '$.BillingMode') = 'PROVISIONED' or json_extract_scalar(t.billing_mode_summary, '$.BillingMode') is NULL )and (pr._cq_id IS not NULL and pw._cq_id IS not NULL) then 'pass'
        ELSE 'fail'
        END
            AS status
FROM
