...
FROM {{ full_table_name("aws_dynamodb_tables") }} t
 ...
 {% macro athena__point_in_time_recovery(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'DynamoDB tables should have point-in-time recovery enabled' as title,
    ...
    t.arn as resource_id,
  case when
    json_extract_scalar(b.point_in_time_recovery_description, '$.PointInTimeRecoveryStatus') is distinct from 'ENABLED'
    then 'fail'
    else 'pass'
  end as status
FROM aws_dynamodb_tables t
