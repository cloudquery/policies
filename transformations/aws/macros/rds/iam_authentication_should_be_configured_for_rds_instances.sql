...
{% macro default__iam_authentication_should_be_configured_for_rds_instances(framework, check_id) %}{% endmacro %}

{% macro bigquery__iam_authentication_should_be_configured_for_rds_instances(framework, check_id) %}
select
    '{{framework}}' as framework,
    '{{check_id}}' as check_id,
    'IAM authentication should be configured for RDS instances' as title,
...
{% macro athena__iam_authentication_should_be_configured_for_rds_instances(framework, check_id) %}
SELECT
    '{{framework}}' AS framework,
    '{{check_id}}' AS check_id,
    'IAM authentication should be configured for RDS instances' AS title,
