...
{% macro athena__elbv1_desync_migration_mode_defensive_or_strictest(framework, check_id) %}
select
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'Classic Load Balancer should be configured with defensive or strictest desync mitigation mode' as title,
    ...
    arn as resource_id,
    case
        WHEN json_extract_scalar(aa, '$.Value') in ('defensive', 'strictest') THEN 'pass'
        ELSE 'fail'
    END as status
FROM
    aws_elbv1_load_balancers as lb,
    unnest(cast(json_extract(attributes, '$.AdditionalAttributes') as array(json))) as t(aa)
WHERE
    json_extract_scalar(aa, '$.Key') = 'elb.http.desyncmitigationmode'
{% endmacro %}