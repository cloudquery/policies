...
{% macro athena__autoscaling_group_elb_healthcheck_required(framework, check_id) %}
select
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
  'Auto Scaling groups associated with a Classic Load Balancer should use load balancer health checks' AS "title",
  ...
  arn AS resource_id,
  case
  when cardinality(load_balancer_names) = 0 and cardinality(target_group_arns) = 0 then 'fail'
  when health_check_type not like '%ELB%' then 'fail'
  else 'pass'
  END
    AS status
