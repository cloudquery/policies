...
{% macro bigquery__clusters_should_have_7_days_backup_retention(framework, check_id) %}
select 
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'Amazon DocumentDB clusters should have an adequate backup retention period' as title,
...
{% macro athena__clusters_should_have_7_days_backup_retention(framework, check_id) %}
select 
    '{{framework}}' As framework,
    '{{check_id}}' As check_id,
    'Amazon DocumentDB clusters should have an adequate backup retention period' as title,
