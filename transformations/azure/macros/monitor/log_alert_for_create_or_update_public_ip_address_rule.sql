{% macro monitor_log_alert_for_create_or_update_public_ip_address_rule(framework, check_id) %}
  {{ return(adapter.dispatch('monitor_log_alert_for_create_or_update_public_ip_address_rule')(framework, check_id)) }}
{% endmacro %}

{% macro default__monitor_log_alert_for_create_or_update_public_ip_address_rule(framework, check_id) %}{% endmacro %}

{% macro postgres__monitor_log_alert_for_create_or_update_public_ip_address_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
...
conditions AS (
    SELECT
        ...
        location = 'global'
            AND enabled
            AND equals = 'Microsoft.Network/publicIPAddresses/write'
...
SELECT
    ...
    'Ensure that Activity Log Alert exists for Create or Update Public IP Address rule (Automated)' AS title,
...
{% macro snowflake__monitor_log_alert_for_create_or_update_public_ip_address_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
...
{% macro bigquery__monitor_log_alert_for_create_or_update_public_ip_address_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
