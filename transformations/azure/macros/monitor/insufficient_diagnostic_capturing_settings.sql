...
{% macro bigquery__monitor_insufficient_diagnostic_capturing_settings(framework, check_id) %}
WITH diagnostic_settings AS (
      SELECT
          subscription_id,
          id,
          CAST( JSON_VALUE(logs.enabled) AS BOOL) AS enabled,
          JSON_VALUE(logs.category) AS category
      FROM {{ full_table_name("azure_monitor_subscription_diagnostic_settings") }},
      UNNEST(JSON_QUERY_ARRAY(properties.logs)) AS logs
  ),
  required_settings AS (
      SELECT *
      FROM diagnostic_settings
