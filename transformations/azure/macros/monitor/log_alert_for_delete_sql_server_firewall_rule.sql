{% macro monitor_log_alert_for_delete_sql_server_firewall_rule(framework, check_id) %}
  {{ return(adapter.dispatch('monitor_log_alert_for_delete_sql_server_firewall_rule')(framework, check_id)) }}
{% endmacro %}

{% macro default__monitor_log_alert_for_delete_sql_server_firewall_rule(framework, check_id) %}{% endmacro %}

{% macro postgres__monitor_log_alert_for_delete_sql_server_firewall_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
...
conditions AS (
    SELECT
        ...
        location = 'global'
            AND enabled
            AND equals = 'Microsoft.Sql/servers/firewallRules/delete'
...
SELECT
    ...
    'Ensure that Activity Log Alert exists for Delete SQL Server Firewall Rule' AS title,
...
{% macro snowflake__monitor_log_alert_for_delete_sql_server_firewall_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
...
{% macro bigquery__monitor_log_alert_for_delete_sql_server_firewall_rule(framework, check_id) %}
WITH fields AS (
    SELECT
        subscription_id,
        id,
