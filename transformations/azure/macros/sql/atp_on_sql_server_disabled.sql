...
{% macro bigquery__sql_atp_on_sql_server_disabled(framework, check_id) %}
SELECT
  s.id AS server_id,
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
  ...
  s.subscription_id,
  case
    when JSON_VALUE(atp.properties.state) is distinct from 'Enabled'
    then 'fail' else 'pass'
  end
FROM {{ full_table_name("azure_sql_servers") }} s
    JOIN {{ full_table_name("azure_sql_server_advanced_threat_protection_settings") }} atp ON s._cq_id = atp._cq_parent_id
{% endmacro %}