...
{% macro bigquery__sql_postgresql_allow_access_to_azure_services_enabled(framework, check_id) %}
SELECT
  aps.id AS server_id,
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
  ...
  case
    when apsfr.name = 'AllowAllAzureIps'
      OR (JSON_VALUE(apsfr.properties.startIPAddress) = '0.0.0.0'
      AND JSON_VALUE(apsfr.properties.endIPAddress) = '0.0.0.0')
    then 'fail' else 'pass'
  end
FROM {{ full_table_name("azure_postgresql_servers") }} aps
    LEFT JOIN {{ full_table_name("azure_postgresql_server_firewall_rules") }} apsfr ON
        aps._cq_id = apsfr._cq_parent_id
{% endmacro %}