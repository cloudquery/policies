{% macro sql_postgresql_log_checkpoints_disabled(framework, check_id) %}
  {{ return(adapter.dispatch('sql_postgresql_log_checkpoints_disabled')(framework, check_id)) }}
{% endmacro %}

{% macro default__sql_postgresql_log_checkpoints_disabled(framework, check_id) %}{% endmacro %}

{% macro postgres__sql_postgresql_log_checkpoints_disabled(framework, check_id) %}
WITH value_check AS (
...
{% macro snowflake__sql_postgresql_log_checkpoints_disabled(framework, check_id) %}
WITH value_check AS (
    SELECT aps._cq_id, apsc.properties:value as value
    FROM azure_postgresql_servers aps
        LEFT JOIN azure_postgresql_server_configurations apsc ON
...
{% macro bigquery__sql_postgresql_log_checkpoints_disabled(framework, check_id) %}
WITH value_check AS (
