...
{% macro bigquery__sql_data_encryption_off(framework, check_id) %}
SELECT
  asd.id AS database_id,
  '{{framework}}' As framework,
  '{{check_id}}' As check_id,
  ...
  s.subscription_id,
  CASE
    WHEN JSON_VALUE(tde.properties.state) IS DISTINCT FROM 'Enabled'
    THEN 'fail'
    ELSE 'pass'
  END
FROM {{ full_table_name("azure_sql_servers") }} s
    LEFT JOIN {{ full_table_name("azure_sql_server_databases") }} asd ON
        s._cq_id = asd._cq_parent_id
    LEFT JOIN {{ full_table_name("azure_sql_transparent_data_encryptions") }} tde ON
        asd._cq_id = tde._cq_parent_id
WHERE asd.name != 'master'
{% endmacro %}