...
FROM azure_keyvault_keyvaults akv
    ...
    WHERE (akv.properties ->> 'enableRBAC')::boolean IS NOT distinct from TRUE
{% endmacro %}
...
FROM azure_keyvault_keyvaults akv
    JOIN azure_keyvault_keys akvk
      ON akv._cq_id = akvk._cq_parent_id
      where akv.properties:enableRBAC::boolean = TRUE
{% endmacro %}
...
FROM {{ full_table_name("azure_keyvault_keyvaults") }}  akv
    JOIN {{ full_table_name("azure_keyvault_keys") }} akvk
      ON akv._cq_id = akvk._cq_parent_id
      where CAST( JSON_VALUE(akv.properties.enableRBAC) AS BOOL) = TRUE
{% endmacro %}