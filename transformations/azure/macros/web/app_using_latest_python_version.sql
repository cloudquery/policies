...
SELECT
       ...
       subscription_id                                                        AS subscription_id,
       CASE
        WHEN JSON_VALUE(properties.SiteConfig.linuxFxVersion) not like 'PYTHON%' then 'pass'
        WHEN JSON_VALUE(properties.SiteConfig.linuxFxVersion) = 'PYTHON|3.9' then 'pass'
        ELSE 'fail'
        END AS status
