{% macro sql_mysql_audit_log_enabled(framework, check_id) %}
  {{ return(adapter.dispatch('sql_mysql_audit_log_enabled')(framework, check_id)) }}
{% endmacro %}

{% macro default__sql_mysql_audit_log_enabled(framework, check_id) %}{% endmacro %}

{% macro postgres__sql_mysql_audit_log_enabled(framework, check_id) %}
WITH value_check AS (
    ...
    FROM azure_mysql_servers msql_s
        ...
        WHERE msql_sc.name = 'audit_log_enabled'
)
...
SELECT
  ...
  s.subscription_id,
  case
    when v.value IS NULL OR lower(v.value) != 'on'
      then 'fail' else 'pass'
  end
...
{% macro snowflake__sql_mysql_audit_log_enabled(framework, check_id) %}
WITH value_check AS (
    SELECT msql_s._cq_id, msql_sc.properties:value as value
    FROM azure_mysql_servers msql_s
        LEFT JOIN azure_mysql_server_configurations msql_sc ON
...
{% macro bigquery__sql_mysql_audit_log_enabled(framework, check_id) %}
WITH value_check AS (
...
SELECT
  ...
  s.subscription_id,
  case
    when v.prop IS NULL OR lower(v.prop) != 'on'
      then 'fail' else 'pass'
  end
