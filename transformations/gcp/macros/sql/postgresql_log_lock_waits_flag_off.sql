{% macro sql_postgresql_log_lock_waits_flag_off(framework, check_id) %}
  {{ return(adapter.dispatch('sql_postgresql_log_lock_waits_flag_off')(framework, check_id)) }}
{% endmacro %}

{% macro default__sql_postgresql_log_lock_waits_flag_off(framework, check_id) %}{% endmacro %}

{% macro postgres__sql_postgresql_log_lock_waits_flag_off(framework, check_id) %}
select
...
{% macro snowflake__sql_postgresql_log_lock_waits_flag_off(framework, check_id) %}
 WITH 
    instance_flags as (
    select
        f.value as value             
...
select
    ...
    FROM gcp_sql_instances gsi
    LEFT JOIN 
    instance_flags AS f ON f.value:name ='log_lock_waits'
{% endmacro %}

{% macro bigquery__sql_postgresql_log_lock_waits_flag_off(framework, check_id) %}
WITH 
    instance_flags as (
    select
        f as value
...
select
    ...
    FROM {{ full_table_name("gcp_sql_instances") }} gsi
    LEFT JOIN 
    instance_flags AS f ON JSON_VALUE(f.value.name) ='log_lock_waits'
{% endmacro %}